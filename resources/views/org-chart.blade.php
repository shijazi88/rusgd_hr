        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="flex items-center gap-5">
                ...
                <div class="flex items-center gap-5 text-[11px] font-semibold text-slate-400">
                    ...
                    <span class="flex items-center gap-1.5">
                        <span class="w-2.5 h-2.5 rounded-full bg-indigo-400 shadow-[0_0_6px_rgba(99,102,241,.5)]"></span>
                        الموظف
                    </span>
                </div>
                <div class="w-px h-5 bg-[#1e3a5f]"></div>
                <input x-model="search" @input="doSearch()"
...
/* ── Tier colour tokens ──────────────────────────────────────────── */
:root {
    --c0: #f59e0b;  --c0-bg: rgba(245,158,11,.09);  --c0-ring: rgba(245,158,11,.35);
    --c1: #14b8a6;  --c1-bg: rgba(20,184,166,.09);   --c1-ring: rgba(20,184,166,.30);
    --c2: #818cf8;  --c2-bg: rgba(99,102,241,.09);   --c2-ring: rgba(99,102,241,.28);
    --line: var(--rs-line);
}
...
.oc-card {
    position: relative;
    min-width: 164px; max-width: 196px;
...
.oc-card.show    { opacity: 1; transform: translateY(0) scale(1); }
.oc-card:hover   { transform: translateY(-5px) scale(1) !important; }
...
.oc-card.l0:hover { box-shadow: 0 10px 40px rgba(245,158,11,.22); border-color: var(--c0); }

.oc-card.l1 { border-color: var(--c1-ring); }
.oc-card.l1:hover { box-shadow: 0 10px 36px rgba(20,184,166,.18); border-color: var(--c1); }

.oc-card.l2 { border-color: var(--c2-ring); background: var(--c-card); }
.oc-card.l2:hover { box-shadow: 0 10px 32px rgba(99,102,241,.15); border-color: var(--c2); }
...
.l0 .oc-badge { background: var(--c0); color: #000; }
.l1 .oc-badge { background: var(--c1); color: #002e2b; }
.l2 .oc-badge { background: #6366f1; color: #fff; }

/* ── Sequence number ─────────────────────────────────────────────── */
.oc-seq {
    position: absolute; top: 10px; left: 11px;
    font-size: 9px; font-weight: 800; opacity: .45;
}
.l0 .oc-seq { color: var(--c0); }
.l1 .oc-seq { color: var(--c1); }
.l2 .oc-seq { color: var(--c2); }
...
.oc-dot.active     { background:#22c55e; box-shadow:0 0 0 2.5px rgba(34,197,94,.2); }
.oc-dot.on_leave   { background:#f59e0b; box-shadow:0 0 0 2.5px rgba(245,158,11,.2); }
.oc-dot.terminated { background:#ef4444; }
.oc-dot.inactive   { background:#475569; }
...
.oc-card:hover .oc-av { transform: scale(1.1); }

.l0 .oc-av { background: var(--c0-bg); color: var(--c0); border-color: var(--c0); box-shadow: 0 0 16px rgba(245,158,11,.25); }
.l1 .oc-av { background: var(--c1-bg); color: var(--c1); border-color: var(--c1); }
.l2 .oc-av { background: var(--c2-bg); color: var(--c2); border-color: #6366f1; }

/* ── Text ────────────────────────────────────────────────────────── */
.oc-name  { font-size: 13px; font-weight: 700; color: var(--c-text-1); line-height: 1.3; }
.oc-title { font-size: 11px; color: var(--c-text-3); margin-top: 3px; }
.oc-dept  {
    display: inline-block; font-size: 10px; font-weight: 600;
    padding: 2px 10px; border-radius: 999px; margin-top: 8px;
}
.l0 .oc-dept { background: var(--c0-bg); color: var(--c0); }
.l1 .oc-dept { background: var(--c1-bg); color: var(--c1); }
.l2 .oc-dept { background: var(--c2-bg); color: var(--c2); }

/* ── SVG connector glow on hover ─────────────────────────────────── */
.oc-path { transition: stroke .3s, stroke-width .3s; }

/* ── Search states ───────────────────────────────────────────────── */
.oc-col.dim > .oc-card        { opacity: .1  !important; transform: none !important; box-shadow: none !important; }
.oc-col.hi  > .oc-card        { box-shadow: 0 0 0 2px #f59e0b, 0 8px 30px rgba(245,158,11,.3) !important; border-color: #f59e0b !important; }
</style>
    ...
    return {
        ...
        buildCol(node, depth) {
            ...
            if (node.children && node.children.length > 0) {
                ...
                childRow.style.cssText = `display:flex;gap:${gap}px;justify-content:center;align-items:flex-start;padding-top:56px;`;
        ...
        drawConnectors() {
            ...
            wrapper.querySelectorAll('.oc-card[data-child-ids]').forEach((parentCard, pi) => {
                let childIds;
                try { childIds = JSON.parse(parentCard.dataset.childIds); } catch(e) { return; }
                ...
                childIds.forEach((cid, ci) => {
                    ...
                    path.style.transition = `stroke-dashoffset 0.65s cubic-bezier(.4,0,.2,1) ${drawDelay}ms`;
        ...
        doSearch() {
            ...
            document.querySelectorAll('.oc-col').forEach(col => {
                if (!q) { col.classList.remove('dim', 'hi'); return; }
